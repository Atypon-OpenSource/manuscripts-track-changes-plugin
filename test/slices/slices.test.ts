...
describe('slices.test', () => {
  ...
  test('should correctly wrap copy-pasted slice with track markup', async () => {
    const tester = setupEditor({
      ...
      .paste(new Slice(Fragment.from(schema.text('inserted')), 0, 0), 18, 18)
      .paste(new Slice(Fragment.from(schema.text('replaced')), 0, 0), 5, 14)
  ...
  test('should prevent replacing of blockquotes and break the slice into parts instead', async () => {
    const tester = setupEditor({
      ...
      .paste(
        new Slice(Fragment.from(utils.createBlockquote(schema, 'open-end blockquote')), 0, 2),
        0,
        17
      )
      ...
      .paste(
        new Slice(Fragment.from(utils.createBlockquote(schema, 'open-start blockquote')), 2, 1),
        55,
        74
      )
  ...
  test('should track pasted slices and prevent deletion of non-inserted content', async () => {
    const tester = setupEditor({
      ...
      .paste(
        new Slice(
          Fragment.from([
            utils.createParagraph(schema, 'inserted paragraph'),
            utils.createBlockquote(schema, 'inserted blockquote'),
          ]),
          1,
          1
        ),
      ...
      // deleted.
      .paste(
        new Slice(Fragment.from([utils.createBlockquote(schema, 'pasted open blockquote')]), 2, 2),
        11,
        31
      )
      ...
      .paste(
        new Slice(Fragment.from(utils.createBlockquote(schema, '2nd inserted blockquote')), 0, 2),
        71,
        112
      )
  ...
  test.skip('todo bugs', async () => {
    const tester = setupEditor({
      ...
      .paste(
        new Slice(Fragment.from(utils.createBlockquote(schema, 'delete inside blockquote')), 1, 1),
        16,
        50
      )
