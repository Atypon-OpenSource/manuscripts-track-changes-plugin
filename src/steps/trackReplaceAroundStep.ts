...
export function trackReplaceAroundStep(
  ...
  const {
    ...
    newSliceContent,
    steps: deleteSteps,
  } = deleteAndMergeSplitNodes(from, to, { start: gapFrom, end: gapTo }, newTr.doc, newTr, oldState.schema, attrs, slice)
  ...
  if (gap.size > 0 || (!structure && newSliceContent.size > 0) || tr.getMeta(TrackChangesAction.updateMetaNode)) {
