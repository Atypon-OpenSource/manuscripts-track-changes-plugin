...
export function deleteNode(node: PMNode, pos: number, tr: Transaction) {
  ...
  */
  ...
  const canMergeToNodeAbove = resPos.parent !== tr.doc && resPos.nodeBefore && node.isBlock && node.firstChild?.isText
 ...
 * @param deleteAttrs
 */
export function deleteOrSetNodeDeleted(node: PMNode, pos: number, newTr: Transaction, deleteAttrs: NewDeleteAttrs) {
  const dataTracked = getBlockInlineTrackedData(node)
  const inserted = dataTracked?.find((d) => d.operation === CHANGE_OPERATION.insert && d.status === CHANGE_STATUS.pending)
  ...
  const newDeleted = deleted ? { ...deleted, updatedAt: deleteAttrs.updatedAt } : addTrackIdIfDoesntExist(deleteAttrs)
