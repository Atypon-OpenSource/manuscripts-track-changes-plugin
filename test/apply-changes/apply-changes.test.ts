  ...
  test('should delete reference change', async () => {
    ...
    tester.cmd((state, dispatch) => {
      ...
      const referenceId = tester
        .trackState()
        ?.changeSet?.pending.find((change) => change.dataTracked.operation === 'reference' && change.dataTracked.referenceId === nodeSplitChange?.id)
      if (nodeSplitChange && referenceId) {
        trackCommands.setChangeStatuses(CHANGE_STATUS.rejected, [referenceId.id, nodeSplitChange.id])(state, dispatch)
      }
    })
