  ...
  changes.forEach((c) => {
    ...
    switch (c.type) {
      ...
      case 'update-node-attrs':
        ...
        const newDataTracked = [...oldDataTracked.filter((d) => !oldUpdate || d.id !== oldUpdate.id || lastChangeRejected)]
        const newUpdate =
          oldUpdate && oldUpdate.status !== CHANGE_STATUS.rejected
            ...
            : addTrackIdIfDoesntExist(trackUtils.createNewUpdateAttrs(emptyAttrs, lastChangeRejected ? oldAttrs : c.node.attrs))
        // Dont add update changes if there exists already an insert change for this node
        if (
          (JSON.stringify(oldAttrs) !== JSON.stringify(c.newAttrs) || c.node.type === c.node.type.schema.nodes.citation) &&
